...
import { ledgerStats, readLedger, recordFetch, LEDGER_READ_MAX, LEDGER_READ_DEFAULT } from './ledger.js'
      ...
      output: {
        ...
        render(_args, value) {
          ...
          for (const source of value.sources) {
            ...
            if (source.access) lines.push(`   取数边界：${source.access}${['export-import', 'unsupported'].includes(source.access) ? '（stash_fetch 会拒绝，只能人工导出后走 stash_files）' : ''}`)
      ...
      async execute(args) {
        ...
        for (const source of sources) {
          enriched.push({
            ...
            fetchRefused: FETCH_REFUSED_ACCESS.has(source.access ?? ''),
