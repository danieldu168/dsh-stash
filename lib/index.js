...
import { registerPortabilityRoute } from './portability-route.js'
import { registerStashCommand } from './command.js'
import { registerStashPrompt } from './prompt.js'
  ...
  try {
    ...
    if (typeof ctx.inject === 'function') {
      ctx.inject(['webServer'], (hostCtx) => {
        ...
        // 迁移端点（导出 / 导入 / 清空）。与 /stash 命令调同一批 portability 函数，
        // 区别只是入口：命令给人用，这条给面板用。**值都不经过浏览器。**
        try {
          const disposePortability = registerPortabilityRoute(hostCtx, { getCredentials })
          if (disposePortability) disposers.push(disposePortability)
          else warn('[dsh-stash] webServer 已就绪但 register 不可用，面板迁移区块不可用（/stash 命令仍可用）')
        } catch (error) {
          warn(`[dsh-stash] 迁移端点注册失败：${describeError(error)}`)
        }
      })
      ...
      if (catalogTool) {
        ctx.inject(['commands'], (hostCtx) => {
          try {
            registerStashCommand(hostCtx, { catalogTool, getCredentials, disposers, warn })
          } catch (error) {
      ...
      // 每步之前的按需资源线索。位置选 `agent/pre-step`——harness 自己注入 AGENTS.md
      // 用的就是这条接缝，它带着本轮用户的 messages，所以能**只在相关时**注入。
      // 相比之下 systemPrompt.section 看不到用户说了什么，只能每步常驻报状态。
      try {
        const disposePrompt = registerStashPrompt(ctx, { warn })
        if (disposePrompt) disposers.push(disposePrompt)
      } catch (error) {
        warn(`[dsh-stash] 按需资源线索注册失败：${describeError(error)}`)
      }
    } else {
