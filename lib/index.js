/**
 * dsh-stash —— 个人持久文库（Profile Bundle 的 host 半边）。
 *
 * 它只做三件事：
 *   1. 把外部数据库/资料库/文献库登记成一份人可编辑的注册表（~/.dsh/stash/sources.mjs）
 *   2. 把"取数时最容易算错的地方"固化成代码保证，而不是文档里的叮嘱
 *   3. 把上面两件事暴露为三个 Model Tool：stash_catalog / stash_fetch / stash_files
 *
...
export function apply(ctx, _config = {}) {
  ...
  try {
    ...
    if (typeof ctx.inject === 'function') {
      ...
      warn('[dsh-stash] 本 ctx 没有 inject，无法注册 webServer 路由与 /stash 命令（5 个工具不受影响）')
    }
  } catch (error) {
