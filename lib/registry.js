...
function collectRequestPlaceholders(request) {
  ...
  const walk = (value) => {
    if (typeof value === 'string') {
      // 同时认 {name} 与可选写法 {?name}——required 若点名了可选参数，不算写错。
      for (const match of value.matchAll(/\{(\?)?([A-Za-z_][A-Za-z0-9_]*)\}/g)) names.add(match[2])
      return
    }
    if (Array.isArray(value)) {
      for (const item of value) walk(item)
...
export function validateSourceDeep(source) {
  ...
  if (source.kind === 'remote' && source.handler === 'http') {
    ...
    // 可选占位符 {?name} 只在 request.query 里有意义——它靠"缺参数则整键丢弃"生效，
    // 而 url 丢不掉占位符、headers / body 也没有这个语义。写错地方就当场拦下，别等取数时才怪。
    const misplacedOptional = new Set()
    const walkOptional = (value) => {
      if (typeof value === 'string') {
        for (const match of value.matchAll(/\{\?([A-Za-z_][A-Za-z0-9_]*)\}/g)) misplacedOptional.add(match[1])
        return
      }
      if (Array.isArray(value)) {
        for (const item of value) walkOptional(item)
        return
      }
      if (value && typeof value === 'object') {
        for (const item of Object.values(value)) walkOptional(item)
      }
    }
    walkOptional({ url: request.url, headers: request.headers, body: request.body, paginate: request.paginate })
    if (misplacedOptional.size > 0) {
      const list = [...misplacedOptional].map((name) => `{?${name}}`).join(' / ')
      issues.push({
        level: 'error',
        message: `${at}：可选占位符 ${list} 只能出现在 request.query 的值里——`
          + '缺参数时它靠"整个查询键丢弃"生效，而 url / headers / body 没有这个语义。',
      })
    }
  }
