...
/**
 * 可选占位符 `{?name}`：**只在 `request.query` 的值里合法**（别处的用法由 registry 在登记时拦下）。
 *
 * 语义是显式的：参数缺失时**整个查询键被丢弃**，而不是代入空串。
 * 后者会发出 `category=eq.` 这种看起来合法、实际必被上游拒的畸形串——
 * 而"可选过滤器"根本没法用 `required` 表达（那会把它变成必填）。
 */
const OPTIONAL_PLACEHOLDER = /\{\?([A-Za-z_][A-Za-z0-9_]*)\}/g

export const GUARANTEES = [
  '凭据值只出现在真实请求中，回传的 URL / headers / body 一律脱敏为 ***引用名***',
  'required 里声明的参数缺失时在发请求前报错，不靠服务端 400 才发现',
  '分页有页数上限，避免一次工具调用打出无界请求',
  'cacheTtlMs 声明的缓存寿命由本处理器强制执行：过期即重新取数，不把陈旧数据当新鲜的用',
]
...
function substitute(value, env, secrets, redact) {
  if (typeof value === 'string') {
    ...
    return value
      .replace(OPTIONAL_PLACEHOLDER, (_match, token) => {
        const resolved = env[token]
        return resolved === undefined || resolved === null ? '' : String(resolved)
      })
      .replace(PLACEHOLDER, (_match, token) => {
        if (token.startsWith('credential:')) {
          const ref = token.slice('credential:'.length)
          return redact ? `***${ref}***` : (secrets.get(ref) ?? '')
        }
...
/** 值里只要有一个可选占位符的参数缺失，整个查询键就该被丢弃。 */
function optionalParamMissing(value, env) {
  if (typeof value !== 'string') return false
  for (const match of value.matchAll(OPTIONAL_PLACEHOLDER)) {
...
function buildUrl(template, query, redact, env, secrets) {
  ...
  for (const [key, raw] of Object.entries(query ?? {})) {
    // 可选占位符缺参数 → 整键丢弃。这一步必须在代入之前做：
    // `eq.{?category}` 代入后是 `eq.`，非空，会被下面那道空值守卫放过去。
    if (optionalParamMissing(raw, env)) continue
  ...
  try {
    for (let index = 0; index < (paginate ? maxPages : 1); index += 1) {
      ...
      const response = await fetchJson(url, {
        ...
        host,
        // 缓存寿命：注册表声明了才生效。0 / 未声明 = 永不过期（0.6.0 及以前的行为）。
        cacheTtlMs: Number(request.cacheTtlMs ?? 0) || 0,
        // 只捞注册表点名的响应头（配额计数器一类）；不点名就不带出。
        captureHeaders: Array.isArray(request.captureHeaders) ? request.captureHeaders : [],
      })
